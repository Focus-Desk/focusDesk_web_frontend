...
const FaqItem = memo(({ faq, isOpen, toggle, index }: { faq: any; isOpen: boolean; toggle: (id: number) => void; index: number }) => (
  <motion.div
    ...
    className={`group relative rounded-[2.5rem] overflow-hidden border transition-all duration-700 ${isOpen
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-6">
          <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all duration-500 scale-90 group-hover:scale-100 ${isOpen ? "bg-blue-600 text-white shadow-lg" : "bg-white/5 text-blue-400"
            }`}>
            <HelpCircle className="w-6 h-6" />
          </div>
          <h3 className={`font-[700] text-lg tracking-tight transition-colors duration-500 ${isOpen ? "text-blue-300" : "text-white/90"
            }`}>
            {faq.question}
          </h3>
        </div>
...
export default function CtaSectionComponent() {
  ...
  return (
    <section id="cta" className="relative pt-24 pb-32 px-6 md:px-12 scroll-m-20 overflow-hidden bg-[#020617]">
